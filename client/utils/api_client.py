...
class FastAPIClient:
    ...
    def generate_itinerary(self, planning_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        try:
            endpoint = f"{self.base_url}/api/v1/generate_itinerary"
            response = self.session.post(endpoint, json=planning_data, timeout=30)
            if response.status_code == 200:
                data = response.json()
                return data.get("itinerary")  # 👈 ahora devuelve solo el itinerario
            else:
    ...
    def chat_with_mouse(self, message: str, context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        try:
            endpoint = f"{self.base_url}/api/v1/chat_with_agent"
            
            # Extraer solo las preferencias del usuario del contexto (itinerario)
            user_preferences = None
            if context and isinstance(context, dict):
                # Si el contexto tiene las preferencias directamente
                if all(key in context for key in ['destination', 'duration_days', 'budget_range']):
                    user_preferences = {
                        "destination": context.get("destination"),
                        "duration_days": context.get("duration_days", context.get("total_days", 3)),
                        "budget_range": context.get("budget_range", "medio"),
                        "children_ages": context.get("children_ages", []),
                        "interests": context.get("interests", ["general"]),
                        "family_size": context.get("family_size", 2)
                    }
                # Si es un itinerario completo, buscar en las preferencias originales
            ...
            print(f"DEBUG: Enviando chat payload: {json.dumps(payload, indent=2)}")
            
            response = self.session.post(endpoint, json=payload, timeout=15)
            if response.status_code == 200:
                data = response.json()
                return {"message": data.get("response"), "sender": "mouse"}
            else:
                self.logger.error(f"Error en chat API: {response.status_code} - {response.text}")
